...
const rpcCallHandlers = async (io, socket) => {
  socket.on('rpc:request', (rpcRequest) => {
    const { senderSocket, receiverSocket } = (0, socketio_handlers_1.findRPCSockets)(
      socket.data.podId,
  ...
  socket.on('rpc:response', (rpcResponse) => {
    const { senderSocket = null, receiverSocket = null } = (0, socketio_handlers_1.findRPCSockets)(
      socket.data.podId,
      socket.data.connectionType,
    );
    try {
