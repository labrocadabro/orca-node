    ...
    socket.on('orcaPulse:init', (payload) => {
        ...
        if (podSpec) {
            ...
            cmClient.imageUrlValid(podImageUrl).then((isImageUrlValid) => {
                if (isImageUrlValid) {
                        ...
                        if (isImageExist) {
                            ...
                            cmClient.exists(podId).then((isPodAvailable) => {
                                if (isPodAvailable) {
                                    ...
                                    cmClient.create(podId, podImageUrl, [], [
                    ...
                    logger.error(`User container imageURL #${podImageUrl} is not valid, It should be start with registry domainName or "localhost" when image is in the localhost registry`);
                }
            });
        }
        socket.on('disconnect', (reason) => {
            ...
            if (reason == 'client namespace disconnect') {
                logger.log(`orca - pulse client successfuly  disconnected after success[reason: ${reason}], shutting down pod with Id ${podId} `);
