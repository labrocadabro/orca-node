...
let ContainerLifecycleManagerService = class ContainerLifecycleManagerService {
    ...
    async imageUrlValid(imageUrl) {
        const hostnameRegexStr = "((([-a-z0-9]{1,63}\\.)*?[a-z0-9]([-a-z0-9]{0,253}[a-z0-9])?\\.[a-z]{2,63})|((\\d{1,3}\\.){3}\\d{1,3}))(:\\d{1,5})?((\\/|\\?)((%[0-9a-f]{2})|[-\\w\\+\\.\\?\\/@~#&=])*)?$";
    ...
    async createByPodSpec(podSpec) {
        try {
            const { stdout, stderr } = await exec(`echo "${podSpec}" | podman play kube --network slirp4netns:allow_host_loopback=true -`);
            this.logger.log(stdout);
        }
        catch (error) {
            this.logger.error(error.stderr);
