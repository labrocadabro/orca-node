  ...
  async install() {
    ...
    try {
      if (fs.existsSync(setupPath)) {
        ...
        throw new Error(
          `Can't find Podman setup package! Path: ${setupPath} doesn't exists.`,
        );
      }
    } catch (err) {
  ...
  async install() {
    ...
    try {
      if (fs.existsSync(pkgPath)) {
        try {
          const { stdout } = await exec(`open ${pkgPath} -W`);
          this.logger.log(stdout);
        } catch (err) {
          this.logger.error(err.stderr);
        }
        ...
        throw new Error(
          `Can't find Podman package! Path: ${pkgPath} doesn't exists.`,
        );
      }
    } catch (err) {
      this.logger.error('Error during install!');
