...
async function download_bnaries(binaryUrl, binaryPath) {
    axios({
        ...
        response.data.pipe(fs.createWriteStream(binaryPath))
            .on('finish', () => {
...
let WinInstaller = class WinInstaller {
    ...
    async install() {
        ...
        const setupPath = path.resolve((0, util_1.getAssetsFolder)(), `podman-${os.platform()}-${os.arch()}.exe`);
        try {
...
let MacOSInstaller = class MacOSInstaller {
    ...
    async install() {
        ...
        await download_bnaries(binaryUrl, binaryPath);
        const pkgPath = path.resolve((0, util_1.getAssetsFolder)(), `podman-${os.platform}-${os.arch}.pkg`);
        try {
